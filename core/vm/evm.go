...
func (evm *EVM) AuthCreate(caller ContractRef, data []byte, gas uint64, endowment *uint256.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	...
	nonce := evm.StateDB.GetNonce(eoaAddr)
	if nonce+1 < nonce {
		return nil, common.Address{}, gas, ErrNonceUintOverflow
	}
	evm.StateDB.SetNonce(eoaAddr, 0)
	r, address, overGas, err := evm.create(caller, codeAndHash, gas, endowment, eoaAddr, AUTHCREATE)
	evm.StateDB.SetNonce(eoaAddr, nonce+1)
	return r, address, overGas, err
}
